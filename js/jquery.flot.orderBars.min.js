...
*/
(function(m){m.plot.plugins.push({init:function(m){function q(a,c){for(var d=[],b=0;b<a.length;b++)if(Array.isArray(a[b].data)&&0<a[b].data.length){var f="number"==typeof a[b].data[0][c]?a[b].data[0][c]:0,g="number"==typeof a[b].data[a[b].data.length-1][c]?a[b].data[a[b].data.length-1][c]:a[b].data.length-1;d[0]=0<d.length&&d[0]<=f?d[0]:f;d[1]=1<d.length&&d[1]>=g?d[1]:g}return d}function v(a,c){var d=a.bars.order,b=c.bars.order;return d<b?-1:d>b?1:0}function r(a,c,d){for(var b=0;c<=d;c++)b+=a[c].bars.barWidth+
